...
func point_to_string_inner_split_seven_one{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    in: felt
) -> (first_bytes: felt, last_byte: felt) {
    alloc_locals;
...
func point_to_string_one{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(point: EcPoint) -> (
    ...
    let (
        pk_string_one: felt,
        pk_string_two: felt,
        pk_string_three: felt,
        pk_string_four: felt,
        pk_string_five: felt,
        pk_string_six: felt,
    ) = BigInt3_to_64bit(point.x);
...
func point_to_string_two{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    in_byte: felt, point: EcPoint
) -> (p1: felt, p2: felt, leftover_bytes: felt) {
    alloc_locals;
    let (y_as_uint384) = bigint3_to_uint384(point.y);
    let (_, y_mod_2) = uint384_lib.unsigned_div_rem(y_as_uint384, Uint384(d0=2, d1=0, d2=0));
    ...
    let (str_two_six_bytes, str_two_two_bytes) = point_to_string_inner_split_six_two(
        pk_string_four
    );
...
func hash_points{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    H: EcPoint, Gamma: EcPoint, U: EcPoint, V: EcPoint
) -> (res: Uint256) {
    alloc_locals;
